...
func TestNewElf(t *testing.T) {
	...
	fakeMtime := time.Date(2000, 1, 1, 0, 0, 0, 0, time.UTC)
	require.NoError(t, os.Chtimes(elfPath, fakeMtime, fakeMtime))
...
func TestNewElfNotElf(t *testing.T) {
	...
	assert.ErrorContains(t, err, "file is not an ELF")
}
...
func TestElfSearchSymbolsMultiple(t *testing.T) {
	...
	symbols, err := e.SearchSymbols(ctx, targets, elf.SHT_SYMTAB)
	require.NoError(t, err)
	require.Equal(t, []elf.Symbol{
		{Name: "SSL_read", Info: 0x12, Section: elf.SHN_UNDEF + 1, Value: 0x2000, Size: 0x50},
		{Name: "SSL_write", Info: 0x12, Section: elf.SHN_UNDEF + 1, Value: 0x3000, Size: 0x60},
		{Name: "hello_world", Info: 0x12, Section: elf.SHN_UNDEF + 1, Value: 0x4000, Size: 0x70},
	}, symbols)
}
...
func TestMatchStrategySuffix(t *testing.T) {
	result := MatchSymbol("crypto_SSL_read", []SymbolSearch{
		{Name: "_read", MatchStrategy: MatchStrategySuffix},
	})
	assert.True(t, result)
}

func TestMatchStrategyContains(t *testing.T) {
	result := MatchSymbol("my_SSL_read_impl", []SymbolSearch{
		{Name: "SSL_read", MatchStrategy: MatchStrategyContains},
	})
	assert.True(t, result)
}
