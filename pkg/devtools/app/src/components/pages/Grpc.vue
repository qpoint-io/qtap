<template>
  <div class="h-full flex flex-col">
    ...
    <SplitPanel
      ref="splitPanelRef"
      storage-key="devtools_grpc_panel_width"
      :default-width="40"
      :min-width="10"
      :max-width="90"
    ...
    >
      <template #left>
        ...
        >
          ...
          <div
            ...
            :style="{ height: `${ITEM_HEIGHT}px` }"
            :class="{
              'bg-dt-bg-selected dark:bg-dt-bg-dark-selected': params.grpc_id === request.requestId,
            }"
          >
            <template v-for="column in displayedColumns" :key="column.key">
              ...
              <div
                ...
                class="px-2 py-1.5 border-r border-dt-border-light dark:border-dt-border-dark-light font-semibold truncate"
                :class="getGrpcStatusClass(request.grpcStatus)"
                :style="{ width: `${column.width}px`, minWidth: `${column.minWidth}px` }"
              >
                {{ formatGrpcStatus(request) }}
              </div>
      ...
      <template #right>
          ...
          <div class="flex-1">
            <div class="flex items-center gap-3 mb-2">
              ...
              <span class="font-semibold text-sm">
                {{ selectedRequest!.grpcMethod || selectedRequest!.path }}
              </span>
              <span
                class="font-semibold text-sm"
                :class="getGrpcStatusClass(selectedRequest!.grpcStatus)"
              >
                {{ formatGrpcStatus(selectedRequest!) }}
              </span>
              <span class="text-xs text-dt-text-secondary dark:text-dt-text-dark-secondary">
                {{ formatDuration(selectedRequest!.duration) }}
              </span>
            </div>
            <div class="text-xs font-mono text-dt-text-secondary dark:text-dt-text-dark-secondary truncate">
        ...
        <div class="flex bg-dt-bg-secondary dark:bg-dt-bg-dark-secondary border-b border-dt-border-dark dark:border-dt-border-dark-dark">
          <button
            @click="activeDetailTab = 'rpc'"
            class="px-4 py-2 text-xs relative cursor-pointer"
            :class="{
              'text-dt-text-primary dark:text-dt-text-dark-primary': activeDetailTab === 'rpc',
              'text-dt-text-secondary dark:text-dt-text-dark-secondary hover:text-dt-text-primary dark:hover:text-dt-text-dark-primary': activeDetailTab !== 'rpc',
            }"
          >
            RPC
            <div
              v-if="activeDetailTab === 'rpc'"
              class="absolute bottom-0 left-0 right-0 h-0.5 bg-dt-accent dark:bg-dt-accent-dark-blue"
            ></div>
          </button>
          <button
            @click="activeDetailTab = 'status'"
            class="px-4 py-2 text-xs relative cursor-pointer"
            :class="{
              'text-dt-text-primary dark:text-dt-text-dark-primary': activeDetailTab === 'status',
              'text-dt-text-secondary dark:text-dt-text-dark-secondary hover:text-dt-text-primary dark:hover:text-dt-text-dark-primary': activeDetailTab !== 'status',
            }"
          >
            Status
            <div
              v-if="activeDetailTab === 'status'"
              class="absolute bottom-0 left-0 right-0 h-0.5 bg-dt-accent dark:bg-dt-accent-dark-blue"
            ></div>
          </button>
          <button
        ...
        <div class="flex-1 overflow-y-auto p-3 bg-dt-bg-primary dark:bg-dt-bg-dark-primary">
          <!-- RPC Tab -->
          <div v-if="activeDetailTab === 'rpc'">
            <div class="mb-4">
              <h3 class="text-xs font-semibold text-dt-text-secondary dark:text-dt-text-dark-secondary mb-2">Path</h3>
              <div class="bg-dt-bg-secondary dark:bg-dt-bg-dark-secondary p-2 rounded border border-dt-border-light dark:border-dt-border-dark-light relative">
                <Clipboard
                  :content="selectedRequest!.path"
                  class="absolute top-2 right-2"
                />
                <pre class="text-xs font-mono text-dt-text-primary dark:text-dt-text-dark-primary whitespace-pre-wrap break-all">{{ selectedRequest!.path }}</pre>
              </div>
            </div>

            <div class="mb-4">
              <h3 class="text-xs font-semibold text-dt-text-secondary dark:text-dt-text-dark-secondary mb-2">Service</h3>
              <div class="bg-dt-bg-secondary dark:bg-dt-bg-dark-secondary p-2 rounded border border-dt-border-light dark:border-dt-border-dark-light">
                <p class="text-xs font-mono text-dt-text-primary dark:text-dt-text-dark-primary">{{ selectedRequest!.grpcService || '-' }}</p>
              </div>
            </div>

            <div class="mb-4">
              <h3 class="text-xs font-semibold text-dt-text-secondary dark:text-dt-text-dark-secondary mb-2">Method</h3>
              <div class="bg-dt-bg-secondary dark:bg-dt-bg-dark-secondary p-2 rounded border border-dt-border-light dark:border-dt-border-dark-light">
                <p class="text-xs font-mono text-dt-text-primary dark:text-dt-text-dark-primary">{{ selectedRequest!.grpcMethod || '-' }}</p>
              </div>
            </div>
          ...
          <!-- Status Tab -->
          <div v-else-if="activeDetailTab === 'status'">
            <div class="mb-4">
              <h3 class="text-xs font-semibold text-dt-text-secondary dark:text-dt-text-dark-secondary mb-2">gRPC Status</h3>
              <div class="bg-dt-bg-secondary dark:bg-dt-bg-dark-secondary p-2 rounded border border-dt-border-light dark:border-dt-border-dark-light flex items-center gap-2">
                <span
                  class="text-xs font-semibold px-2 py-0.5 rounded"
                  :class="getGrpcStatusBadgeClass(selectedRequest!.grpcStatus)"
                >
                  {{ selectedRequest!.grpcStatusName || '-' }}
                </span>
                <span class="text-xs font-mono text-dt-text-secondary dark:text-dt-text-dark-secondary">
                  ({{ selectedRequest!.grpcStatus }})
                </span>
              </div>
            </div>

            <div v-if="selectedRequest!.grpcMessage" class="mb-4">
              <h3 class="text-xs font-semibold text-dt-text-secondary dark:text-dt-text-dark-secondary mb-2">Message</h3>
              <div class="bg-dt-bg-secondary dark:bg-dt-bg-dark-secondary p-2 rounded border border-dt-border-light dark:border-dt-border-dark-light relative">
                <Clipboard
                  :content="selectedRequest!.grpcMessage"
                  class="absolute top-2 right-2"
                />
                <pre class="text-xs font-mono text-dt-text-primary dark:text-dt-text-dark-primary whitespace-pre-wrap break-all">{{ selectedRequest!.grpcMessage }}</pre>
              </div>
            </div>
          </div>
...
import { useGrpc } from '@/composables/grpc'
...
const defaultColumns: ColumnConfig[] = [
  ...
  { key: 'method', label: 'Method', width: 150, minWidth: 60, visible: true, resizable: true },
  { key: 'service', label: 'Service', width: 200, minWidth: 80, visible: true, resizable: true },
  { key: 'status', label: 'Status', width: 120, minWidth: 60, visible: true, resizable: true },
...
const {
  ...
  storageKey: 'devtools_grpc_columns',
  defaultColumns,
})
...
const CUSTOM_RESIZE_KEY = 'devtools_grpc_custom_resize'
const hasCustomResizing = ref(localStorage.getItem(CUSTOM_RESIZE_KEY) === 'true')

const expandServiceToFillSpace = (force = false) => {
  if (!scrollContainer.value) return
  if (!force && hasCustomResizing.value) return

  const serviceCol = columns.value.find(col => col.key === 'service')
  if (!serviceCol || !serviceCol.visible) return
  ...
  const targetServiceWidth = Math.max(serviceCol.minWidth, availableWidth - otherColumnsWidth)
  serviceCol.width = targetServiceWidth
}
...
const handleAutoFit = (key: string) => {
  ...
  requestAnimationFrame(() => expandServiceToFillSpace(true))
}

function handleClear() {
  grpcStore.clearRequests()
}
...
function handleResetColumns() {
  ...
  requestAnimationFrame(() => expandServiceToFillSpace())
}

const params = useUrlParams()
const activeDetailTab = ref<'rpc' | 'status' | 'timing' | 'process'>('rpc')

watch(scrollContainer, (container) => {
  if (container) {
    containerHeight.value = container.clientHeight
    nextTick(() => expandServiceToFillSpace(true))
  }
}, { immediate: true })
...
// Color-code by gRPC status
const getGrpcStatusClass = (grpcStatus: string): string => {
  if (grpcStatus === '0') {
    return 'text-method-post-light dark:text-method-post-dark'  // green — OK
  }
  if (grpcStatus === '1') {
    return 'text-dt-text-secondary dark:text-dt-text-dark-secondary'  // gray — CANCELLED
  }
  return 'text-dt-status-error dark:text-dt-status-dark-error'  // red — error
}

const getGrpcStatusBadgeClass = (grpcStatus: string): string => {
  if (grpcStatus === '0') {
...
const formatGrpcStatus = (request: GrpcRequest): string => {
  if (request.grpcStatusName && request.grpcStatus) {
    return `${request.grpcStatusName} (${request.grpcStatus})`
  }
  if (request.grpcStatus) {
    return request.grpcStatus
  }
  return '-'
}
...
const selectRequest = (request: GrpcRequest) => {
  params.grpc_id = request.requestId || ''
  activeDetailTab.value = 'rpc'
}

const closeRequest = () => {
  delete params.grpc_id
}
...
const selectedRequest = computed(() => {
  const requestId = params.grpc_id as string
