<template>
  <div class="h-full flex flex-col">
    ...
    <SplitPanel
      ref="splitPanelRef"
      storage-key="devtools_postgres_panel_width"
      :default-width="40"
      :min-width="10"
      :max-width="90"
      ...
      <template #left>
        ...
        >
          ...
          <div
            ...
            :style="{ height: `${ITEM_HEIGHT}px` }"
            :class="{
              'bg-dt-bg-selected dark:bg-dt-bg-dark-selected': params.postgres_id === request.requestId,
            }"
          >
...
import { usePostgres, getStatementType } from '@/composables/postgres'
...
const {
  ...
  storageKey: 'devtools_postgres_columns',
  defaultColumns,
})
...
const CUSTOM_RESIZE_KEY = 'devtools_postgres_custom_resize'
...
const selectRequest = (request: DatabaseRequest) => {
  params.postgres_id = request.requestId || ''
  activeDetailTab.value = 'query'
}

const closeRequest = () => {
  delete params.postgres_id
}
...
const selectedRequest = computed(() => {
  const requestId = params.postgres_id as string
