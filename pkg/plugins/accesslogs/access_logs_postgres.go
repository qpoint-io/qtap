...
type postgresFilterInstance struct {
	...
	commands []*plugins.PostgresCommand
	results  []*plugins.PostgresResult
}
...
func (f *postgresFilterInstance) Destroy() {
	...
	for i, cmd := range f.commands {
		var res *plugins.PostgresResult
		if i < len(f.results) {
			res = f.results[i]
		}

		printer := NewPostgresPrinter(f.format, f.ctx, cmd, res, f.logger, f.writer)
		switch f.mode {
...
func buildPostgresSummary(cmd, direction string, pgCmd *plugins.PostgresCommand, res *plugins.PostgresResult) string {
...
func formatPostgresResultTable(columns []string, rows [][]string, nullFlags [][]bool) string {
	...
	for i, col := range columns {
		...
		sb.WriteString(subsectionStyle.Render(padRightPG(col, widths[i])))
	}
	sb.WriteString("\n")
	...
	for ri, row := range rows {
		...
		for ci := range columns {
			...
			if isNull {
				val = "NULL"
				sb.WriteString(labelStyle.Render(padRightPG(val, widths[ci])))
			} else {
				if ci < len(row) {
					val = row[ci]
				}
				...
				sb.WriteString(valueStyle.Render(padRightPG(val, widths[ci])))
			}
		}
		sb.WriteString("\n")
	}
...
// padRightPG pads a string to the given width with spaces
func padRightPG(s string, width int) string {
	if len(s) >= width {
		return s
	}
...
func getPostgresColorFn(res *plugins.PostgresResult) func(a ...interface{}) string {
	if res == nil {
